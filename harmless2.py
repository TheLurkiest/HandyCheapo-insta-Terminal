...
def playModernJazz(genre,subgenre):
    ...
    while(playerReplyU.count("Q")<=1):
        lcdSays("Choose from the following options to adjust music: D: Decrease volume   U: volume Up   Q: quits music   P: Pause and Play   I: Increase speed   S: Slow down")
        ...
        if (playerReplyU.count("Q")>=1):
            ...
            defaultSongTimer=0
            lcdSays("Enter Q AGAIN to Quit playing music or N to play the next song")
            if(inputMode == "Keyboard"):
            ...
            playerReplyU=playerReply.upper()
            if (playerReplyU.count("Q")>=1):
                lcdSays("Stopping Music...")
            elif (playerReplyU.count("N")>=1):
                lcdSays("Playing Next Song...")
                nextSongNum2=nextSongNum2+1
                songpath=cutSongArr[int(playerReply)+nextSongNum2]
                lcdSays("Now Playing: "+str(cutSongArr[int(playerReply)+nextSongNum2]))
                songpath='/home/pi/Desktop/finalProject/music/'+genre+'/'+subgenre+'/'+randomSong+'.mp3'
                omxprocess = subprocess.Popen(['omxplayer', '-o', 'local', songpath], stdin=subprocess.PIPE, stdout=None, stderr=None, bufsize=0)
                time.sleep(5)
...
inputMode = "Keyboard"#inputMode = "morseCode"# XXX reverse these I mark with XXX back to "morseCode" when you want to regain that option# XXX reverse these I mark with XXX back to "morseCode" when you want to regain that option
...
while (playerReplyU.count("8")< 1):
  ...
  lcdSays(">>MAIN MENU<<")
  if(inputMode == "Keyboard"):
    lcdSays("press \'t\' and hit enter to enter tutorial mode")
    playerReply=input("enter response")
    if(playerReply == "t"):
        helpfulTips=1
    else:
        helpfulTips=0
    ...
    helpfulTips=0
  if(helpfulTips == 1):
    menuSays="hit the primary button at any time to skip over lcd messages.  Use ctrl+c to end the program and ctrl+d to exit the terminal."
    lcdSays(menuSays)
    menuSays="If using a keyboard at startup without a mouse or monitor you will be unable to enter text using the keyboard until you have performed the following precise steps to restart this program.  These steps must be performed IN ORDER: "
    lcdSays(menuSays)
    menuSays="   1) hit the Win key on your keyboard using the left side Win key if available... the right side Win key should also work if it is the only one on your keyboard.   2) hit the down arrow key 5 times.   3) hit the right arrow key 1 time.   4) hit the down arrow key 7 times.   5) hit enter."
    lcdSays(menuSays)
    menuSays="Choose from options using Morse Code: "
    lcdSays(menuSays)
    mainMenuSays="Choose from the Following Menu Options : Option 0- Full access to terminal using keyboard   Option 1- Write to file   Option 2- read from file   Option 3- Perform online search   Option 4- check email   Option 5- send email   Option 6- execute terminal commands   Option 7- play music   Option 8- quit   Option 9- change settings   Option 10- Activate observation and detection mode"
    lcdSays(mainMenuSays)#alright by simply opening up  



  
  lcdSays("Enter a number 0-10 to select a menu option.")
  
  if(helpfulTips == 1):
      lcdSays("Enter 8 or q to quit.  Enter h for helpful hints.  Enter any other key to see menu options listed again.")
  ...
  if (playerReplyU == "10"):
      if(helpfulTips == 1):
          menuSays=("WARNING: make sure not to click anywhere other than the spreadsheet cell otherwise the data will spill out everywhere")
          lcdSays(menuSays)
      if (inputMode != "Keyboard"):
          lcdSays("Sorry but this is currently only designed to function with a keyboard.  You will need to plug one in or have it hook up automatically through bluetooth connection")
      else:
          lcdSays("Choose from the following menu options: 1- Security Mode   2- Gardener Mode   3- Record ALL possible observations that exceed threshold")
          playerReply=input("Select one of these menu options or simply type \"back\" to return to the main menu:")
          playerReplyU=playerReply.upper()
          if(playerReplyU == "3"):
              lcdSays("Please wait...")
              os.system('libreoffice pyAtStartBman/DataObserved.ods')#libreoffice DataObserved9.ods
              lcdSays("Please wait a moment before flicking the center switch on your circuitplayground express microcontroller... doing so will initiate data recording by our device and we don\'t want to do that until our spreadsheet file has finished loading")
              lcdSays("...alright you should be good.  Go ahead and flick the center switch on your microcontroller when you\'re ready to begin recording data.")
          else:
              lcdSays("sorry but no other menu options are currently available at this time")
  if (playerReplyU == "0"):
      if (inputMode != "Keyboard"):
          lcdSays("Sorry but it is too unsafe to enter terminal commands without even a keyboard.  Please change inputMode to Keyboard before attempting to use this function.")
      if(helpfulTips == 1):
          menuSays=("WARNING: giving user full access to terminal commands with very little safety net in place.  Be wary. Type back into the terminal to return to main menu.")
          lcdSays(menuSays)
      lcdSays("Enter a terminal command: ")

      playerReplyU = playerReply.upper()
      while(playerReplyU != "BACK"):
          playerReply=input("Enter a terminal command or back to return to previous menu: ")
          playerReplyU = playerReply.upper()
          if(playerReplyU == "BACK"):
              lcdSays("Returning to previous menu...")
          else:
              os.system('echo \"' + playerReply + '\" > /home/pi/Desktop/finalProject/output1.txt')

              
              fileContent=readFromFile("/home/pi/Desktop/finalProject/output1.txt")
              lcdSays("You entered: "+str(fileContent))
              if(helpfulTips == 1):
                  lcdSays("Are you sure you want to enter that terminal command?  You can answer with yes or no or just y or n.")
              else:
                  lcdSays("Confirm?  y or n")
              playerReply = input("Confirm terminal command execution?")
              playerReplyU=playerReply.upper()
              if (playerReplyU.count("Y")>=1):
                  lcdSays("Understood.  Executing terminal command now.")
                  os.system('script -c \''+str(fileContent)+'\' /home/pi/Desktop/finalProject/input1.txt')# script -c 'ls -l' input2.txt                
                  #os.system(''+str(fileContent))
                  fileContent2=readFromFile("/home/pi/Desktop/finalProject/input1.txt")
                  lcdSays("Result: "+str(fileContent2))
                  os.system('echo \"\" > /home/pi/Desktop/finalProject/input1.txt')                  
